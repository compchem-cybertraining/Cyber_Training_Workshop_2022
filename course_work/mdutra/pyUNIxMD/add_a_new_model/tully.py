...
class Tully(Model):
    ...
    def get_data(self, molecule, base_dir, bo_list, dt, istep, calc_force_only):
        ...
        # Define Hamiltonian
        if x>0:
            H[0][0] = self.A*(1.-np.exp(-self.B*x))
        else:
            H[0][0] = -self.A*(1.-np.exp(self.B*x))
        H[1][1] = -H[0][0]

        H[0][1] = self.C*np.exp(-self.D*x**2)
        H[1][0] = H[0][1]

        # Define a derivative of Hamiltonian
        if x>0:
            dH[0,0]=self.A*self.B*np.exp(-self.B*x)
        else:
            dH[0,0]=self.A*self.B*np.exp(self.B*x)

        dH[1,1]=-dH[0,0]

        dH[0,1]=-2.0*self.C*self.D*x*np.exp(-self.D*x**2)
        dH[1,0]=dH[0,1]
        ...
        Uct = U.conjugate().transpose()

        # Extract adiabatic quantities
        molecule.states[0].energy = evals[0]
        molecule.states[1].energy = evals[1]

        forces = np.dot(Uct,np.matmul(dH,U))
        molecule.states[0].force = forces[0,0]
        molecule.states[1].force = forces[1,1]

        molecule.nac[0, 1, 0, 0] = np.dot(U[:,0].T, np.dot(dH, U[:,1])) /(evals[1]-evals[0])
